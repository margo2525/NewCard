...
function createNewsListItem({ title, headerBgSrc, category, body, date }) {
	...
	newsListItemEl.onclick = (e) => {
		e.currentTarget.style.backgroundColor = "#7FFFD4";
	};
	...
	newsTitleEl.append(newsTitleBgEl, createcontenerNew(title));
	...
	newsTitleBgEl.append(likeBtn);

	const sectionEl = document.createElement("section");
	sectionEl.classList.add("dateDelete");
	newsListItemEl.append(createCategory(category), createNews(body), sectionEl);
	...
	sectionEl.append(createNewsDate(date), removeBtn);
	return newsListItemEl;
}

function createcontenerNew(title) {
	const contenerNewEl = document.createElement("p");
	contenerNewEl.classList.add("contenerNew");
	contenerNewEl.textContent = title;
	return contenerNewEl;
}
